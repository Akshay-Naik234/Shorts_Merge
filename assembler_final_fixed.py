...
Reads numbered MP4 clips from videos/ and narration.mp3.\nUnreadable/corrupt clips are automatically validated and re-encoded when possible.
...
def get_ffmpeg_exe() -> str:
    """Return the FFmpeg executable bundled/used by MoviePy."""
    return imageio_ffmpeg.get_ffmpeg_exe()


def validate_video_file(video_file: Path) -> bool:
    """Check whether FFmpeg can decode at least the first video frame."""
...
def repair_video_file(video_file: Path) -> Path:
    """
    Repair/re-encode a video that FFmpeg cannot decode normally.

    The repaired file is placed beside the original as:
        76_repaired.mp4
    ...
    print(f"       WARNING: {video_file.name} failed FFmpeg decode check.")
    print("       Attempting automatic repair/re-encode...")

    repaired = video_file.with_name(f"{video_file.stem}_repaired.mp4")
    ffmpeg = get_ffmpeg_exe()

    # Use FFmpeg's error-tolerant flags so partially damaged MP4s have
    # the best possible chance of being recovered.
    cmd = [
        ffmpeg,
        "-y",
        "-hide_banner",
    ...
    if result.returncode != 0 or not repaired.exists() or repaired.stat().st_size == 0:
        error_tail = result.stderr[-3000:] if result.stderr else "Unknown FFmpeg error"
        if repaired.exists():
            try:
                repaired.unlink()
            except OSError:
                pass
        raise RuntimeError(
            f"Could not repair {video_file.name}.\\n"
            f"FFmpeg output:\\n{error_tail}\\n\\n"
            f"The source file is likely genuinely corrupted. "
            f"Please regenerate/re-download {video_file.name}."
        )

    if not validate_video_file(repaired):
        try:
            repaired.unlink()
        except OSError:
            pass
        raise RuntimeError(
            f"FFmpeg created {repaired.name}, but the repaired file still "
            f"cannot decode its first frame. Please regenerate {video_file.name}."
        )

    print(f"       REPAIRED: using {repaired.name}")
    return repaired
...
def assemble_video(
    ...
    try:
        ...
        for index, (video_no, video_file) in enumerate(
            ...
            usable_video_file = repair_video_file(video_file)
            clip = VideoFileClip(str(usable_video_file), audio=True)
        ...
        usable_final_video_file = repair_video_file(final_video_file)
        final_source = VideoFileClip(str(usable_final_video_file), audio=True)
